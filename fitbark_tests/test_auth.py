...
class Auth2Test(TestCase):
    ...
    def test_authorize_token_url(self):
        ...
        self.assertEqual(retval[0], 'https://www.fitbark.com/oauth2/authorize?response_type=code&client_id=fake_id&redirect_uri=http%3A%2F%2F127.0.0.1%3A8080&scope=activity+nutrition+heartrate+location+nutrition+profile+settings+sleep+social+weight&state='+retval[1])

    def test_authorize_token_url_with_scope(self):
        # authorize_token_url calls oauth and returns a URL
        fb = FitBark(**self.client_kwargs)
        retval = fb.client.authorize_token_url(scope=self.client_kwargs['scope'])
        self.assertEqual(retval[0], 'https://www.fitbark.com/oauth2/authorize?response_type=code&client_id=fake_id&redirect_uri=http%3A%2F%2F127.0.0.1%3A8080&scope='+ str(self.client_kwargs['scope'][0])+ '&state='+retval[1])
    ...
    def test_auto_refresh_expires_at(self):
        ...
        fb = FitBark(**kwargs)
        profile_url = FitBark.API_ENDPOINT + '/1/user/-/profile.json'
        with requests_mock.mock() as m:
            m.get(
                profile_url,
                text='{"user":{"aboutMe": "python-fitbark developer"}}',
                status_code=200
            )
            token = {
                'access_token': 'fake_return_access_token',
        ...
        self.assertEqual(retval['user']['aboutMe'], "python-fitbark developer")
